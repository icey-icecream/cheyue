  ...
  <div class="Sendprogram">
    ...
    <div class="main">
      ...
      <el-form style="margin-bottom:200px;" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="130px" class="demo-ruleForm">
        ...
        <!-- 专辑类型 -->
         <el-form-item label="专辑类型" prop="region">
        ...
        <el-form-item label="语言">
         ...
         <!-- 节目标签 -->
        <el-form-item label="节目标签" style="margin-top: -24px;" prop="programtag">
 ...
 export default {
    ...
    data() {
      ...
      if (tagsArray === '') {
       ...
       var programImg = (rule,value,callback) => {
      var albumImg = this.$refs.upload.imageUrl
      if (albumImg === '') {
        callback(new Error('请上传专辑封面'));
      }else {
        callback()
      }
      };
      var region = (rule,value,callback) => {
      var chooseRegion = this.ruleForm.region
      if (chooseRegion === '') {
        callback(new Error('请选择专辑类型'));
      }else {
        callback()
      }
      };
    ...
    mounted : function() {
            ...
            this.axios.get(this.common.projectName_ + 'album/albumEcho.action?albumId='+albumId)
                .then((response) => {
                      var result=response.data;
                      //检查后台返回结果
                      var resultMessage = this.common.checkResult(result);
                      if (result.code == 0) {
                          this.ruleForm.name = result.data.albumName;
                          this.$refs.upload.imageUrl = result.data.albumPhotoUrl;
                          this.ruleForm.region = result.data.albumType+'';
                          var albumLanguageType = result.data.albumLanguageType;
                          if (albumLanguageType == 1) {
                            this.activeName = '粤语';
                          } else if (albumLanguageType == 2) {
                            this.activeName = '英语';
                          } else if (albumLanguageType == 3) {
                            this.activeName = '其它';
                          } else {
                            this.activeName = '普通话';
                          }
                          var tagsResult = result.data.albumLabel;
                          this.$refs.tagsinput.dynamicTags = tagsResult.split('|');
                          this.ruleForm.desc = result.data.albumIntroduce;
                          var isPublic =  result.data.isPublic;
                          if (isPublic == 1) {
                             this.$refs.radioSecrect.check(1);
                          } else {
                             this.$refs.radioSecrect.check(0);
                          }
                      } else {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            var params = new URLSearchParams();
             params.append('albumId',this.albumId);
             ...
             this.axios.post(this.common.projectName_ + 'album/updateAlbum.action?',params)
            .then((response) => {
              var result=response.data;
              //检查后台返回结果
              var resultMessage = this.common.checkResult(result);
